...
function init ($views) {
  function printPosts ($list, posts) {
    posts.forEach((post) => {
      ...
      $article.setAttribute('class', 'lhl-news-cards-item animated')

      const $info = document.createElement('div')
      $info.setAttribute('class', 'lhl-news-cards-item-info')
      $article.appendChild($info)

      const $inner = document.createElement('div')
      $inner.setAttribute('class', 'lhl-news-cards-item-inner')
      $info.appendChild($inner)

      const $label = document.createElement('p')
      $label.setAttribute('class', 'lhl-news-cards-item-lead-in')
      ...
      if (post.categories.length <= 0) {
        ...
        post.categories.forEach((category, index) => {
          ...
          $anchor.setAttribute('href', '/news?term=' + category.slug)
      ...
      $title.setAttribute('class', 'lhl-news-cards-item-title')
      $title.innerHTML = '<a href="' + post.link + '">' + post.title + '</a>'
      $inner.appendChild($title)

      const $summary = document.createElement('summary')
      $summary.setAttribute('class', 'lhl-news-cards-item-summary')
      $summary.innerText = post.hero_summary ? post.hero_summary : SUMMARY_PLACEHOLDER
      $inner.appendChild($summary)

      const $date = document.createElement('div')
      $date.setAttribute('class', 'lhl-news-cards-item-date')
      $inner.appendChild($date)

      const $icon = document.createElement('img')
      $icon.setAttribute('class', 'lhl-image lhl-news-cards-item-icon')
      ...
      $link.setAttribute('class', 'lhl-news-cards-item-link')
      $link.setAttribute('href', post.link)
      $article.appendChild($link)

      const $img = document.createElement('img')
      $img.setAttribute('class', 'lhl-image lhl-news-cards-item-image')
  ...
  $views.forEach($view => {
    const $list = $elementChild($view, 'news-list')
    const $loadMore = $elementChild($view, 'load-more')
    let page = 1

    if ($loadMore) {
      $loadMore.addEventListener('click', () => {
        const form = new FormData()
        form.append('action', 'get_news')
